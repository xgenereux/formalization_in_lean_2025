...
Many possible projects in this direction, for more information take a look at
...
* We could use a type such as `AExp` from chapter 2 and similarly for Boolean
  expressions.
...
theorem sillyLoop_from_1_BigStep :
    (sillyLoop, (fun _ ↦ 0)["x" ↦ 1]) ⟹ (fun _ ↦ 0 : State) := by
  rw [sillyLoop]
...
Note that it would prove this example using backwards reasoning - but sometimes that is
not possible.
...
theorem l7_3 : Stmt.whileDo B S ~ Stmt.ifThenElse B (S; Stmt.whileDo B S) Stmt.skip := by
  simp_all only [BigStepEquiv, BigStep_if_Iff, BigStep_seq_Iff, BigStep_skip_Iff]
  intro s t
  apply Iff.intro
  · intro a
  ...
  · intro a
    cases a with
    | inl h => simp_all only [BigStep_while_true_Iff]
    | inr h_1 => simp_all only [not_false_eq_true, BigStep_while_false_Iff]
