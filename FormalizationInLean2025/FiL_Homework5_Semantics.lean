...
/-
## Exercise 7.3.
Define a recursive function `deskip : Stmt → Stmt` that eliminates as many SKIPs as
possible from a statement. For example:
`deskip (Stmt.skip; Stmt.whileDo b (Stmt.assign x a; Stmt.skip)) = Stmt.whileDo b (Stmt.assign x a)`
Prove `deskip S ~ S` by induction on `S`. The following lemma might be useful
to prove in preparation: `lemma l7_5 (hSS : S ~ S') : Stmt.whileDo B S ~ Stmt.whileDo B S'`
-/
...
/-
## Exercise 7.4.
Define a small-step semantics for the evaluation of arithmetic expressions: -/
...
/-
## Exercise 7.6.
Define a new loop construct `doWhile S B` (where `S` is
...
/-
## Exercise 7.7. (Small-Step Semantics)
Let `x : ℕ → Stmt` be an infinite sequence
